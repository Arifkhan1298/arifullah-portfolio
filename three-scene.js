    ...
    try {
      ...
      function updateThemeStyling() {
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
          const i3 = i * 3;
          if (i % 2 === 0) {
            colorArray[i3] = light ? 0.008 : 0.133;
            colorArray[i3 + 1] = light ? 0.518 : 0.827;
            colorArray[i3 + 2] = light ? 0.780 : 0.933;
          } else {
            colorArray[i3] = light ? 0.486 : 0.545;
            colorArray[i3 + 1] = light ? 0.227 : 0.361;
            colorArray[i3 + 2] = light ? 0.929 : 0.965;
          }
        }
        particleGeometry.attributes.color.needsUpdate = true;
        particleMaterial.blending = light ? THREE.NormalBlending : THREE.AdditiveBlending;
        particleMaterial.opacity = light ? 0.85 : 0.75;
        particleMaterial.needsUpdate = true;

        lineMaterial.blending = light ? THREE.NormalBlending : THREE.AdditiveBlending;
        lineMaterial.opacity = light ? 0.28 : 0.22;
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
          ...
          for (let j = i + 1; j < PARTICLE_COUNT; j++) {
            ...
            if (dist < MAX_DISTANCE) {
              ...
              const r1 = (i % 2 === 0) ? (light ? 0.008 : 0.133) : (light ? 0.486 : 0.545);
              const g1 = (i % 2 === 0) ? (light ? 0.518 : 0.827) : (light ? 0.227 : 0.361);
              const b1 = (i % 2 === 0) ? (light ? 0.780 : 0.933) : (light ? 0.929 : 0.965);

              const r2 = (j % 2 === 0) ? (light ? 0.008 : 0.133) : (light ? 0.486 : 0.545);
              const g2 = (j % 2 === 0) ? (light ? 0.518 : 0.827) : (light ? 0.227 : 0.361);
              const b2 = (j % 2 === 0) ? (light ? 0.780 : 0.933) : (light ? 0.929 : 0.965);
  ...
  function initCanvas2DFallback() {
    ...
    function draw2D() {
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        for (let j = i + 1; j < points.length; j++) {
          ...
          if (dist < 110) {
            ...
            ctx.strokeStyle = isLight
              ? `rgba(2, 132, 199, ${0.28 * (1 - dist / 110)})`
              : `rgba(34, 211, 238, ${0.18 * (1 - dist / 110)})`;
            ctx.lineWidth = 0.8;
        ...
        ctx.fillStyle = i % 2 === 0
          ? (isLight ? 'rgba(2, 132, 199, 0.85)' : 'rgba(34, 211, 238, 0.75)')
          : (isLight ? 'rgba(124, 58, 237, 0.85)' : 'rgba(139, 92, 246, 0.75)');
        ctx.fill();
      }
